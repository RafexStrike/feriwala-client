    ...
    <div className="mx-auto w-[min(1180px,calc(100vw-2rem))] py-10">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
        ...
        <aside className="space-y-6">
          ...
          <div className="rounded-[1.5rem] border border-line bg-surface p-6">
            <h3 className="font-medium text-ink mb-4">Shipping Address</h3>
            <address className="text-muted not-italic whitespace-pre-line">{order.shippingAddress}</address>
...
function OrderItemCard({ item }: { item: OrderItem }) {
  return (
    <div className="flex gap-4 rounded-xl border border-line bg-surface p-4">
      <div className="relative h-20 w-20 flex-shrink-0 rounded-lg overflow-hidden bg-canvas">
        <div className="flex h-full items-center justify-center text-muted">
          <Package className="h-8 w-8" />
        </div>
      </div>
      <div className="flex-1 min-w-0 space-y-1">
        <h4 className="font-medium text-ink truncate">{item.name}</h4>
    ...
    <div className="space-y-6">
      {history.slice().reverse().map((entry, index) => (
        <div key={entry.changedAt} className="relative flex gap-4">
          ...
          <div className="relative flex h-10 w-10 items-center justify-center rounded-full bg-sky/10 text-sky flex-shrink-0">
...
function OrderSummary({ subtotal, shipping, total, notes }: { subtotal: number; shipping: number; total: number; notes?: string }) {
  return (
    <div className="rounded-[1.5rem] border border-line bg-surface p-6 sticky top-24">
      ...
      <div className="space-y-3 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-medium text-ink">
            ...
            {subtotal > 0 && subtotal < 100 && <span className="ml-2 text-xs text-muted">(Free over $100)</span>}
          </span>
        </div>
        <Separator />
      ...
      {notes && (
        <div className="mt-4 p-3 rounded-lg bg-canvas">
          <p className="text-sm font-medium text-muted">Order Notes</p>
          <p className="mt-1 text-sm text-ink">{notes}</p>
        </div>
      )}
    </div>
  );
